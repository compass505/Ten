...
`dotnet test` は **224 / 224 green**、Unity PlayMode は **11 / 16**。
2026-09-13 に**絵以外をゲームとして繋いだ**（`TenApp` / `NightSession` / MOD-Present。見た目は仮）。
2026-09-13 に**支配戦略（REQ-056）を測って崩し**（ADR-0023）、**初期視線のずれ**を直し、**承認待ちの ADR を 0 件にした**（本人から全面委任）。
...
**実装が終わったので、`src/` と `unity/Assets/` にコードを書いてよい。**
ただし**テストを書き換えて通すのは今も禁止**（下の「テストファースト」3）。
実装で通らないテストが出たら、止めて ADR に理由を残す
（前例: [ADR-0020](docs/10_requirements/decisions/ADR-0020-test-defects-found-in-implementation.md)）。
