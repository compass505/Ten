...
| TC-105 | REQ-002 | Input | 可動範囲を超えるドラッグ | `Look` | **設定した可動範囲に丸められる。**さらに、**範囲を変えるとその範囲で丸められる**（角度がコードに焼き込まれていない） |
...
## TC-105 の期待値を書き換えた（2026-09-08）

**もとは「左右 ±55°、上下 −15〜+30° に丸められる」と具体値を書いていた。**
これは [test_first.md](../../00_process/test_first.md) 4.1 と衝突している。
同節は**「首の可動角度」をバランス値として挙げ、「期待値に書かない」**と定めている
（[ADR-0012](../../10_requirements/decisions/ADR-0012-balance-vs-tests.md)）。

書いたままだと、**可動範囲を調整するたびにテストが落ちる。**
それは ISS-10（テスト書き換え禁止 × バランス調整の衝突）が起きる形そのもの。

**性質ベースに直した。**可動範囲を注入し、
(1) 出力が範囲に収まること (2) **範囲を変えれば丸め先も変わること**を見る。
(2) があるので「たまたま通る」実装（角度の焼き込み）は落ちる。

角度の実際の値は [setting.md](../../20_basic_design/setting.md) 3 節と balance.md にある。

## 境界層のうち Unity が要るもの / 要らないもの

**端末から来る値を引数で受け取る形にしたので、大半は `dotnet test` で走る**
（[architecture.md](../../20_basic_design/architecture.md) のアセンブリの分け方）。

| TC | Unity が要るか |
| --- | --- |
| TC-098〜102（Clock） | **要らない。**`Consume(deltaSeconds)` が経過時間を引数で受け取る |
| TC-103〜107（Input） | **要らない。**接触を `PointerSample` として値で受け取る |
| TC-108, 110〜112（Storage） | **要らない。**`MemoryStorage` に差し替える |
| TC-109（原子的書き込み） | **要る。**実際に落として前の版が残るかを見る |
| TC-113 / 114（Calendar） | **要らない。**`BoardDateRule` が時刻を引数で受け取る |
| TC-115〜119（Share / Power / 権限） | **要る。**ビルドと実機を検査する |
