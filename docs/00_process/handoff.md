...
測って分かった事実が出たとき
状態: **2026-09-06 起草。**フェーズ 3 完了 + フェーズ 4 のテストケース発番まで完了
...
| --- | --- |
| 0 仕組み | 完了 |
| 1 要件定義 | 完了。REQ-001〜058 / NFR-001〜009 が 2026-09-02 確定。**REQ-059 確定 / 060・061・062 は暫定** |
| 2 基本設計 | **完了。**architecture / screens / balance / data_model / setting / diagnosis の 6 本 |
| 3 詳細設計 | **完了。**16 モジュール + types.md。公開 IF 確定 |
| 4 テスト作成 | **TC-001〜163 を発番済み。テストコードは 1 行も無い**（.NET SDK が無いため） |
| 5 実装 | 未着手。**`src/` は空でよい** |

**止まっている理由は 2 つだけ。**

1. **承認が 2 件**（ADR-0015 / 0016）。[ADR-0005](../10_requirements/decisions/ADR-0005-agent-roles.md) により
   エージェントは自分が書いたものを承認しない
2. **この端末に .NET SDK が無い。**フェーズ 4 の DoD「テストが**全て落ちる**」を確認できない

**それ以外の 153 件の TC は承認待ちに依存していない。**SDK さえ入れば今すぐ書き始められる。
...
| # | 決まったこと | 誰が | 現状 |
| --- | --- | --- | --- |
| H-01 | **寝たふりに失敗しても親は動く**（案 Y）。閉眼中に視界が動いても、それが成功の通知にならない | 本人（2026-09-06） | **[ADR-0017](../10_requirements/decisions/ADR-0017-parent-moves-on-failure.md) 起草済み・承認待ち。**screens.md の D-12 と矛盾しているので、承認まで D-12 が有効 |
...
```
承認 2 件（ADR-0015 / 0016）──┐
                              ├─▶ 暫定 REQ-060/061/062 が確定 ──▶ TC-152〜163 が確定
ADR-0017 の承認 ──────────────┘                                        │
                                                                       ▼
.NET SDK を入れる ──▶ 純粋層のテストを書く（TC-001〜007 の乱数から）──▶ 全部落ちるのを確認
                                                                       │
                                                          （フェーズ 4 DoD 達成）
                                                                       ▼
                                                                    実装へ
...
- TC-001〜007（乱数）は [tests/vectors/rng.json](../../tests/vectors/rng.json) が既にあるので、
  SDK が入り次第いちばん先に書ける
