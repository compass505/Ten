...
**承認待ちは 2 件（ADR-0018 / 0019）。**フェーズ 3 完了 + フェーズ 4 のテストケース発番まで完了し、
...
**どちらも親の 3D モデル制作（Codex 担当）を止めている。**
`scratch/visual/parent/` は r4 から r55 まで 50 周以上回って一度も完成していない。

| ADR | 決めること | 止まっているもの |
| --- | --- | --- |
| [ADR-0018](../10_requirements/decisions/ADR-0018-parent-is-mother.md) | **親を「母」として確定する**（黒髪ロング・30 前後）。[setting.md](../20_basic_design/setting.md) 2 節の「名前も性別も明示しない」を覆す | モデルシートの確定 → r56 以降の制作すべて |
| [ADR-0019](../10_requirements/decisions/ADR-0019-parent-model-acceptance.md) | **合格条件を「元絵と一致」からゲーム内の見え方へ移す**。SET-01（作り込み度）も決着 | 周回の終了条件。これが無いと r56 も終わらない |

**覆すコストが 1 日を超えるのは ADR-0018 のほう。**モデル完成後に性別を変えると、顔・髪・4 段階のブレンドシェイプ・服が作り直しになる。
