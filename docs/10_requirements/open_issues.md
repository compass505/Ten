...
| ISS-20 | 寝たふり失敗時に親が起こす動きの正体（抱き上げるのか、布団を直すのか）。**成功時と判別できない長さ・向きにできるか** | **未解決 / 中**。[ADR-0017](decisions/ADR-0017-parent-moves-on-failure.md) は**これを未解決のまま Accepted**（2026-09-06）。判別できないことが REQ-016 の成立条件で、**紙の上では判定できない**（O-09 と同じ計測で見る） | — || ISS-07 | 実行ループの自動化をどこまで上げるか | 保留（実装フェーズで判断） | — |
...
**未解決で残っているのは ISS-07 / 11 / 12 / 15 / 20 の 5 件。**
このうち ISS-20 は ADR-0017 が未解決のまま受容したもので、実機でしか判定できない。
```
